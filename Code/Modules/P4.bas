...
Public Sub Wykop_i_odwoz()
    ...
    cEleP4.ElementP4 "Wykop i odwóz", "m3", "=" & colT(R_CUT), z_zie
    Wykop_R
    Odwoz_R
End Sub
Public Sub Zasyp()
...
Public Sub Prefabrykat()
    ...
    If InStr(Cells(CalcRow_Table, CalcIndex(C_DESCRIPTION)).Text, "s³up") <> 0 Then
        Prefabrykat_M_V "SLP"
    ElseIf InStr(Cells(CalcRow_Table, CalcIndex(C_DESCRIPTION)).Text, "œciana typu Filigran") <> 0 Then
...
Public Sub Mur()
    ...
    cEleP4.ElementP4 "Œciana murowana", "m2", "=" & colT(R_AREA), z_mur
...
Public Sub Rob_Pref()
    ...
    If InStr(Cells(CalcRow_Table, CalcIndex(C_DESCRIPTION)).Text, "œciana typu Filigran") <> 0 Then PREF_R "FIL-SC", R_AREA
